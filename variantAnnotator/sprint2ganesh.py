...
def get_vep_annotations(genomic_hgvs, genome_build):
    """
    ...
    # Choose the correct genomic HGVS for the genome build
    ...
    if not genomic_description:
        logging.error("No valid genomic HGVS found for the selected genome build.")
        return {}

    # Log the URL and parameters for debugging
    logging.debug(f"Querying VEP with genomic HGVS: {genomic_description}")

    response = requests.get(url, params={"hgvs": genomic_description}, headers=headers)

    if response.status_code == 200:
        annotations = response.json()
        # Extract REVEL and SPICE AI scores
...
def main():
    ...
    # Get VEP annotations
    annotations = get_vep_annotations(genomic_hgvs, genome_build)
    ...
    # Create the dictionary with all the gathered data
    result = {
        "variant": variant,
